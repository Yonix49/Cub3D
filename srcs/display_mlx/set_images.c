...
int	set_images(t_data *data)
{
	...
	if (!set_main_img(data))
		return (0);
	if (!load_image(data, data->all->textures[0], data->cord.path_texture_no, &img))
        return (0);
    if (!load_image(data, data->all->textures[1], data->cord.path_texture_so, &img))
        return (0);
...
int	load_image(t_data *data, int *texture, char *path, t_img *img)
{
	...
	y = -1;
	img->img = mlx_xpm_file_to_image(data->all->mlx, path, &img->img_width, &img->img_height);
	if (!img->img)
        return (0);
